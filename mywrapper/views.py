...
def addstudentaccount(request):
	...
	if request.method == 'POST':
		...
		profile = Profile(user=user,is_teacher=False,is_administrator=False,is_student=True, student_teacher_id = studentID,phoneNumber=studentPhoneNumber,emailID=studentEmailID)
		profile.save()
	return Response({'id':-1 ,'status': 'GET request not supported'},status=status.HTTP_400_BAD_REQUEST)
